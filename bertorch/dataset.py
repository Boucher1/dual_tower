...
class ClasDataset(Dataset):
    def __init__(
            self, texts: Union[List[str], List[Tuple[str, str]]], labels: List[int], tokenizer,
...
class ClasPredDataset(Dataset):
    def __init__(
            self, texts: Union[List[str], List[Tuple[str, str]]], tokenizer,
            max_seq_length: int = 512
    ):
        self.data = tokenizer(texts, truncation=True, max_length=max_seq_length)
...
class PairWithLabelDataset(Dataset):
    def __init__(
        ...
        self.data = tokenizer(text_a_list, truncation=True, max_length=32)
        self.pair_data = tokenizer(text_b_list, truncation=True, max_length=128)
        self.labels = labels
