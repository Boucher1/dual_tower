...
# @Author  : hehl
# @Software: PyCharm
# @File    : model_hehl_all.py
# coding=utf-8
import sys

sys.path.append("/bertorch_main\\bertorch")
...
class SentenceBERT(BaseModel):
    ...
    def run_epoch(self, train_dataloader, dev_dataloader, device, epoch, epochs):
        ...
        for step, batch in enumerate(train_dataloader, start=1):
            ...
            loss = self.loss_op(y_pred, labels.float()).mean()
            loss.backward()
            ...
            if step % 100 == 0:
                ...
                #
                # # 将变为三分类 计算acc f1
                # acc = metrics.accuracy_score(predicted,
                #                              np.where(labels.cpu().data.numpy() == 0.7, 2, labels.cpu().data.numpy()))
                # f1 = metrics.f1_score(predicted,
                #                       np.where(labels.cpu().data.numpy() == 0.7, 2, labels.cpu().data.numpy()),
                #                       average="weighted")
                ...
                logger.info(
                    " batch: %d, loss: %.5f,MSE: %.5f,speed: %.2f step/s"
                    % (step, loss, MSE, 100 / time_diff))
                tic_train = time.time()
                losses = []
        MSE = evaluate(self, dev_dataloader, device)
        return device
