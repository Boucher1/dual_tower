...
tokenizer = BertTokenizer.from_pretrained('../../bertorch_main/checkpoint')
...
def create_score_function(path):
    train = open(f"my_data/SBERT_{path}_topK_NoCons_type.csv", "w", encoding="utf-8", newline="")
    ...
    # with open(f"../../Datasets/WQSP/test.json", "r", encoding="utf-8") as f:
        ...
        for i in data["Questions"]:
            ...
            if InferentialChain is not None:
                ...
                """把实体类型加进去，还没写完，看一下需不需要加[unused0]"""
                # 主题实体类型
...
def only_true(path):
    train = open(f"my_data/SBERT_{path}_topK_NoCons_type_onlytrue.csv", "w", encoding="utf-8", newline="")
    train_w = csv.writer(train, delimiter='\t')
