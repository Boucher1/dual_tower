...
goldDataFile = "../WebQSP.test.json"
# predAnswersFile = "Pred.sem.json"
predAnswersFile = "../predAnswer.json"
def main():
    # if len(sys.argv) != 3:
    #     print ("Usage: python eval.py goldData predAnswers")
    #     sys.exit(-1)

    goldData = json.loads(open(goldDataFile, encoding="utf-8").read())
    predAnswers = json.loads(open(predAnswersFile, encoding="utf-8").read())
    ...
    print ( "Average precision over questions: %.5f" % (precSum / total))
    print ( "Average recall over questions: %.5f" % (recSum / total))
    print  ("Average f1 over questions (accuracy): %.5f" % (f1sum / total))
    print  ("F1 of average recall and average precision: %.5f" % (2 * (recSum / total) * (precSum / total) / (recSum / total + precSum / total)))
    print  ("True accuracy (ratio of questions answered exactly correctly): %.5f" % (numCorrect / total))
