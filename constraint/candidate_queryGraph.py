...
def create(name, linux=False, k=3, threshold=0.5):
    entity_type, test_predEntity = create_negQG(name, linux)
    ...
    if linux:
        data_path = f"../WQSP/WebQSP.{name}.json"
    else:
    ...
    with open(data_path, "r", encoding="utf-8") as f:
        ...
        for i in data["Questions"]:
            ...
            w_json = open(f"WQSP_candidate_queryGraph_json/{name}_top{k}/{QuestionId}.json", "w", encoding="utf-8")
            path_info = {}
            ...
            if i["Parses"][0]["PotentialTopicEntityMention"] and i["Parses"][0]["TopicEntityMid"] and i["Parses"][0][
                ...
                """正样本的候选查询图就是把SPARQL窃取它的查询路径"""
                PosQueryGraph = clean_sparql(topicEntityID, Sparql)
                path_info["Sparql"] = Sparql
                path_info["PosQueryGraph"] = PosQueryGraph
                ...
                if one_hop_rels:
                    for one_hop_rel in one_hop_rels:  # 第一跳的路径扩展
                        ...
                        preAnswer = get_answer(topicEntityID, g_1hop)  # 判断是否能够查询出正确的答案
                        if preAnswer:
                            q_g = {"path": g_1hop, "score": f"{one_hop_rel[1]}", "type": 0,
                                   "label": CalculatePRF1(preAnswer, Answers)}
                            if g_1hop == PosQueryGraph or g_1hop == PosQueryGraph[:-1]:
                                is_true = True
                        ...
                        if one_hop_entity_constrains:
                            for one_hop_entity_constrain in one_hop_entity_constrains:
                                ...
                                if preAnswer:
                                    ...
                                    # 如果实体约束名称不一样 要拼接多个约束
                                    if g_1hop_c == PosQueryGraph or g_1hop_c == PosQueryGraph[:-1]:
                                        is_true = True
                                    NegQueryGraph.append(q_g)
                        ...
                        if two_hop_rels:
                            for two_hop_rel in two_hop_rels:
                                ...
                                preAnswer = get_answer(topicEntityID, g_2hop)
                                if preAnswer:
                                    q_g = {"path": g_2hop, "score": f"{one_hop_rel[1]}#{two_hop_rel[1]}", "type": 2,
                                           "label": CalculatePRF1(preAnswer, Answers)}
                                    NegQueryGraph.append(q_g)
                                    if g_2hop == PosQueryGraph or g_2hop == PosQueryGraph[:-1]:
                                        is_true = True
                                    ...
                                    for two_hop_entity_constrain in two_hop_entity_constrains_x:
                                        ...
                                        if preAnswer:
                                            q_g = {"path": g_2hop_c, "score": f"{one_hop_rel[1]}#{two_hop_rel[1]}",
                                                   ...
                                                   "label": CalculatePRF1(preAnswer, Answers)}
                                            if g_2hop_c == PosQueryGraph or g_2hop_c == PosQueryGraph[:-1]:
                                                is_true = True
...
if __name__ == '__main__':
    ...
    #     create(name, True, 2, 0.2)
    # create("test", True, 3, 0.2)
    # for i in [1,4,5]:
    #     create("test", True, i, 0.2)
    create("test", True, 2, 0.2)